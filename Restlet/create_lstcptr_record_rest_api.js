...
(url, file, encode, search, record, config) => {
    ...
    RecordRESTlet.prototype.postFunction = function(requestBody) {
        try {
            ...
            if (isUpdate) {
                ...
                recordObj.setValue("custrecord_lstcptr_transaction_type", "1"); // Updated to 2 as per requirement
            ...
            if (Array.isArray(requestBody.items)) {
               ...
               log.audit("Vendor ID", vendorId);
                ...
                if (amount != null && !isNaN(amount) && totalTax != null && !isNaN(totalTax)) {
                    var tranAmount = amount + totalTax;
                    recordObj.setValue({
    ...
    //         log.debug({ title: `${strDebugTitle} - Vendor Search`, details: `Found ${searchResults.length} vendors for name: ${vendorName}` });
