 ...
 *
 * Name:            OCR Vendor Bill Split Screen CS (ab_ocr_vendor_bill_split_screen_cs.js)
 *
 ...
 * @NModuleScope Public
 * @FileName ab_ocr_vendor_bill_split_screen_cs.js
 */
define(['N/format', 'N/ui/dialog', 'N/currentRecord', 'N/log', 'N/search'],
    /**
     * @param {format} format
     * @param {dialog} dialog
     ...
     * @param {search} search
     */  
    function (format, dialog, currentRecord, log, search) 
    {
        var strDebugTitle = "ab_ocr_vendor_bill_split_screen_cs";
        ...
        function pageInit(context) 
        {
            try
            {
                ...
                // Step 1: Extract subsidiary from URL
                var url = window.location.href;
                var urlParams = new URLSearchParams(url.split('?')[1]);
                var subsidiaryId = urlParams.get('subsidiary');
                log.debug({ title: strDebugTitle + ' (pageInit)', details: "Subsidiary from URL: " + subsidiaryId });
                ...
                if (subsidiaryId) {
                    ...
                    if (subsidiaryConfig) {
                        ...
                        // Step 3: Verify and set values to expense sublist
                        var expenseSublist = currentRecord.getSublist({ sublistId: 'expense' });
                        if (expenseSublist) {
                            var expenseLineCount = currentRecord.getLineCount({ sublistId: 'expense' });
                            var hasDepartment = currentRecord.getSublistField({ sublistId: 'expense', fieldId: 'department', line: 0 }) !== null;
                            var hasClass = currentRecord.getSublistField({ sublistId: 'expense', fieldId: 'class', line: 0 }) !== null;
                            var hasLocation = currentRecord.getSublistField({ sublistId: 'expense', fieldId: 'location', line: 0 }) !== null;

                            log.debug({ title: strDebugTitle + ' (pageInit)', details: "Expense sublist fields - Department: " + hasDepartment + ", Class: " + hasClass + ", Location: " + hasLocation });

                            for (var i = 0; i < expenseLineCount; i++) {
                                currentRecord.selectLine({ sublistId: 'expense', line: i });
                                if (hasDepartment && department) currentRecord.setCurrentSublistValue({ sublistId: 'expense', fieldId: 'department', value: department });
                                if (hasClass && className) currentRecord.setCurrentSublistValue({ sublistId: 'expense', fieldId: 'class', value: className });
                                if (hasLocation && location) currentRecord.setCurrentSublistValue({ sublistId: 'expense', fieldId: 'location', value: location });
                                //currentRecord.commitLine({ sublistId: 'expense' });
                                log.debug({ title: strDebugTitle + ' (pageInit)', details: "Set values for expense line " + i + ": Department=" + department + ", Class=" + className + ", Location=" + location });
                            }

                            // Set default values for new expense line
                            currentRecord.selectNewLine({ sublistId: 'expense' });
                            if (hasDepartment && department) currentRecord.setCurrentSublistValue({ sublistId: 'expense', fieldId: 'department', value: department });
                            if (hasClass && className) currentRecord.setCurrentSublistValue({ sublistId: 'expense', fieldId: 'class', value: className });
                            if (hasLocation && location) currentRecord.setCurrentSublistValue({ sublistId: 'expense', fieldId: 'location', value: location });
                           // currentRecord.commitLine({ sublistId: 'expense' });
                            log.debug({ title: strDebugTitle + ' (pageInit)', details: "Set default values for new expense line: Department=" + department + ", Class=" + className + ", Location=" + location });
                        } else {
                            log.error({ title: strDebugTitle + ' (pageInit)', details: "Expense sublist not found" });
                        }

                        // Step 4: Verify and set values to item sublist
                        var itemSublist = currentRecord.getSublist({ sublistId: 'item' });
                        if (itemSublist) {
                            var itemLineCount = currentRecord.getLineCount({ sublistId: 'item' });
                            var hasItemDepartment = currentRecord.getSublistField({ sublistId: 'item', fieldId: 'department', line: 0 }) !== null;
                            var hasItemClass = currentRecord.getSublistField({ sublistId: 'item', fieldId: 'class', line: 0 }) !== null;
                            var hasItemLocation = currentRecord.getSublistField({ sublistId: 'item', fieldId: 'location', line: 0 }) !== null;

                            log.debug({ title: strDebugTitle + ' (pageInit)', details: "Item sublist fields - Department: " + hasItemDepartment + ", Class: " + hasItemClass + ", Location: " + hasItemLocation });

                            for (var j = 0; j < itemLineCount; j++) {
                                currentRecord.selectLine({ sublistId: 'item', line: j });
                                if (hasItemDepartment && department) currentRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'department', value: department });
                                if (hasItemClass && className) currentRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'class', value: className });
                                if (hasItemLocation && location) currentRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'location', value: location });
                                //currentRecord.commitLine({ sublistId: 'item' });
                                log.debug({ title: strDebugTitle + ' (pageInit)', details: "Set values for item line " + j + ": Department=" + department + ", Class=" + className + ", Location=" + location });
                            }

                            // Set default values for new item line
                            currentRecord.selectNewLine({ sublistId: 'item' });
                            if (hasItemDepartment && department) currentRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'department', value: department });
                            if (hasItemClass && className) currentRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'class', value: className });
                            if (hasItemLocation && location) currentRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'location', value: location });
                            // currentRecord.commitLine({ sublistId: 'item' });
                            log.debug({ title: strDebugTitle + ' (pageInit)', details: "Set default values for new item line: Department=" + department + ", Class=" + className + ", Location=" + location });
                        } else {
                            log.error({ title: strDebugTitle + ' (pageInit)', details: "Item sublist not found" });
                        }
                    } else {
                        log.error({ title: strDebugTitle + ' (pageInit)', details: "No subsidiary config found for subsidiary: " + subsidiaryId });
                    }
                } else {
        ...
        function getSubsidiaryConfig(subsidiaryId) {
            try {
                ...
                var searchResult = subsidiarySearch.run().getRange({ start: 0, end: 1 });
                if (searchResult.length > 0) {
                    return {
                        location: searchResult[0].getValue('custrecord_lstcptr_location'),
                        class: searchResult[0].getValue('custrecord_lstcptr_class'),
                        department: searchResult[0].getValue('custrecord_lstcptr_department')
                    };
                }
                return null;
            } catch (err) {
                log.error({ title: strDebugTitle + ' (getSubsidiaryConfig) Error', details: JSON.stringify({ code: err.name, message: err.message }) });
                return null;
            }
        }
        ...
        function applySelectedPdfTextToField(pdfText) 
        {
            try 
            {
                ...
                try 
                {
                    ...
                    {
                        var currencySymbolsPattern = /^[\$\€\¥\£\₹\₽\₺\₩\₫\₪\₦\₱\฿\₲\₡\₭\₮\₸\Br\лв\R$\د.إد.к\.м\.р\.с\₼]+/;
