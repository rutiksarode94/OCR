...
define(['N/search'],
     ...
     */
    function (search) {
        var strDebugTitle = 'lstcptr_vendor_config_cs';
        ...
        function pageInit(context) {
            try {
                ...
                currentRecord.setValue({ fieldId: 'custpage_lstcptr_vendor_configured', value: JSON.stringify(configuredVendors) });
            } catch (err) {
        ...
        function fieldChanged(context) {
            try {
                ...
                if (fieldId === 'custrecord_lstcptr_vendor_con_parent_ven') {
                    var selectedCustomer = currentRecord.getValue({ fieldId: 'custrecord_lstcptr_vendor_con_parent_ven' });
                    var configuredVendors = JSON.parse(currentRecord.getValue({ fieldId: 'custpage_lstcptr_vendor_configured' }));

                    if (configuredVendors.includes(selectedCustomer)) {
                        alert('Please select a different customer, as this one is already configured.');
                        currentRecord.setValue({ fieldId: 'custrecord_lstcptr_vendor_con_parent_ven', value: '' });
                    }
                }
            } catch (err) {
        ...
        function getConfiguredVendors() {
            ...
            try {
                var configuredVendors = search.create({
                    ...
                    filters: [],
                    columns: [
                        search.createColumn({ name: 'custrecord_lstcptr_vendor_con_parent_ven', label: 'Link To Parent Vendor' })
                    ]
                });

                configuredVendors.run().each(function (result) {
                    rtnData.push(result.getValue({ name: 'custrecord_lstcptr_vendor_con_parent_ven' }));
                    return true;
                });
            } catch (err) {
