...
define(['N/url', 'N/https', 'N/search', 'N/record', 'N/file', 'N/runtime', 'N/ui/serverWidget', 'N/encode'],
    ...
    function(url, https, search, record, file, runtime, serverWidget, encode) {
        ...
        function beforeLoad(context) {
            ...
            var ScriptTitle = 'UE Script Debug';
            log.debug({ title: ScriptTitle, details: 'Execution Context: ' + runtime.executionContext });
            log.debug({ title: ScriptTitle, details: 'User Event Type: ' + context.type });


            if (runtime.executionContext !== runtime.ContextType.EMAIL_CAPTURE && runtime.executionContext !== runtime.ContextType.SOAPWEB_SERVICE) {
                log.debug({ title: debugTitle, details: 'Skipping execution: Not Email Capture or Web Service context.' });
                return;
            }

            if (context.type !== context.UserEventType.EDIT) {
                log.debug({ title: debugTitle, details: 'Skipping execution: Not EDIT mode.' });
                return;
            }

            if (!isThereValue(recId)) {
                log.debug({ title: debugTitle, details: 'Skipping execution: Record ID not found.' });
                return;
            }


            // Now your real business logic starts here...
            log.debug({ title: debugTitle, details: 'Proceeding with script execution...' });



            try {
                log.debug({ title: debugTitle, details: `Record Id: ${recId}, Record Type: ${recType}` });
                // Check both fields for file ID
                const fileId = newRecord.getValue({ fieldId: 'custrecord_lstcptr_pdf_file' }) || 
                              newRecord.getValue({ fieldId: 'custrecord_lstcptr_email_pdf_attachment' });
                ...
                if (responseCode === 200) {
                    ...
                    // const requestSuiteletUrl = 'https://tstdrv1423092.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=3928&deploy=1&compid=TSTDRV1423092&ns-at=AAEJ7tMQtmUE-4fpBelRYVbt8BqxgPufa-wgmPRJzv8PTAI2y8s';
                            ...
                            <script>
                                jQuery(document).ready(function() {
                                    ...
                                    try {
                                        ...
                                        alert("Sending file to Nanonets API...");
                                        jQuery.ajax({
                                            ...
                                            success: function(data) {
                                               alert("Nanonets API success: " + JSON.stringify(data));
                                                jQuery.ajax({
                                                    ...
                                                    success: function(response) {
                                                        alert("Suitelet success: " + JSON.stringify(response));
                                                    },
                                                    error: function(xhr, status, error) {
                                                        alert("Suitelet error: " + JSON.stringify(xhr));
                                                    },
                                                    data: JSON.stringify({
                                                        "recordId": "${recId}",
                                                        "recordType": "${recType}",
