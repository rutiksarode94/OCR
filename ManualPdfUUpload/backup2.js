...
define(['N/ui/serverWidget', 'N/file', 'N/https', 'N/log', 'N/record', 'N/search', 'N/encode', 'N/url', 'N/runtime'],
                ...
                document.getElementById("infoBox").style.display = "none"; // Hide the message
                    ...
                    async function uploadFile() 
                    {
                              ...
                              if (!file) {
                               ...
                               // Convert file to base64
                                   ...
                                   // Prepare the form data
                                    ...
                                    {
                                        $.ajax({
                                        url: "${url.resolveScript({ scriptId: 'customscript_upload_bill_to_process_sl', deploymentId: 'customdeploy_upload_bill_to_process_sl' })}",
                                        type: "POST",
                        ...
                        try {
                            ...
                            var formData = new FormData();
                            formData.append("file", file);
                            formData.append('MSsubsidiaryId', 'MSsubsidiaryId');
                            formData.append('MSvendorId', 'MSvendorId');

                            // Call the Suitelet to handle file upload & OCR processing
                            ...
                            if (result.ParsedResults) {
                                alert("OCR Successful! Check console for data.");
                            } else {
                                alert("OCR Failed: " + result.ErrorMessage);
                            }
                        } catch (error) {
                            console.error("Upload Error:", error);
    ...
    <ul>
        ...
        `;
                    ...
                    form.clientScriptModulePath = 'SuiteScripts/OCR Vendor to Bill Process/cs.js'; // Update with correct path
                    ...
                    try {
                        ...
                        if (!context.request.body) {
                            log.error("Error", "Request body is empty");
                            return;
                        }
                        try {
                        ...
                        let subsidiaryId = requestBody?.MSsubsidiaryId || null;
                        let vendorId = requestBody?.MSvendorId || null;
                ...
                function getAuthenticationToken(apiKey) {
                    ...
                    try {
                        ...
                        log.error({ title: strDebugTitle + ' (getAuthenticationToken) Error', details: JSON.stringify({ code: error.name, message: error.message }) });
                    }
