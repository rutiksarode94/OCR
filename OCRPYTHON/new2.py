                    ...
                    for cell in row.cells:
                        if cell["label"] in item and cell["text"]:
                            item[cell["label"]] = (
                                ...
                                if cell["label"] in ["Description", "Line_amount"]
                                else cell["text"]
                            )
                    items.append(item)
    return items

def export_line_items(line_items, output_file="/tmp/line_items.csv"):
    """
    Export line items to a CSV file in the /tmp directory.
    """
    if not line_items:
    ...
    try:
        ...
        print(f"Failed to write to file system: {e}. Logging line items to console instead.")
        for item in line_items:
            print(item)
...
def get_payload(input):
    ...
    payload = {
        ...
        "total": 0,
        "items": get_line_items(inference),  # Pass the inference object here
        "originalfile": [
            {
...
def handler(input):
    try:
        ...
        line_items = data["items"]  # Extract line items

        # Export line items (optional, for debugging or logging)
